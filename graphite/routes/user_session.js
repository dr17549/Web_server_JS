...
router.get('/stories', requiresLogin, async (req, res) => {
  try {
    ...
    //return the ^
    res.render('stories', { title: 'Graphite', stories: 'active', user: req.session.user, data: stories});
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
...
router.post('/stories/:function/:id', requiresLogin, async (req, res) => {
  if(req.params.function == "edit") {
    try {
      ...
      //return the ^
      res.render('new_story', { title: 'Graphite', stories: 'active', user: req.session.user, data: storyData, story_ID: story[0].story_ID});
    } catch (err) {
      res.status(500).json({ message: err.message });
    }
    ...
    try {
      ...
      const stories = await Story.find({user_ID: req.session.user.user_ID});
      //return the ^
      res.render('stories', { title: 'Graphite', stories: 'active', user: req.session.user, created: "deleted", data: stories});
    } catch (err) {
      res.status(500).json({ message: err.message });
    }
  }
...
router.post('/new_story', requiresLogin, async (req, res) => {
  ...
  if(req.body.story_ID >= 0) {
    Story.findOneAndUpdate({story_ID: req.body.story_ID}, {story: JSON.stringify(json)}, function(err, result) {
      if (err) {
        res.send(err);
      } else {
        //res.send(result);
  ...
  try {
    ...
    const stories = await Story.find({user_ID: req.session.user.user_ID});
    //return the ^
    res.render('stories', { title: 'Graphite', stories: 'active', created: req.body.story_ID >= 0 ? "updated" : "created", user: req.session.user, data: stories});
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
});
...
router.get("/new_graph", requiresLogin, async (req, res) => {
  res.render("new_graph", { title: 'Graphite', graphs: 'active', user: req.session.user});
});
